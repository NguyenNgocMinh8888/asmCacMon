...
import fdlhn.sof3021.sd17321.respositories.NVRepo;
import fdlhn.sof3021.sd17321.viewModel.NhanVien;
...
public class NhanVienController {
    ...
    public String update(@Valid @ModelAttribute("nv") NhanVien nv, BindingResult result){
        if(result.hasErrors()){
            return "nhanVien/update";
        }
        for(int i = 0; i <repo.size(); i++){
            if(repo.get(i).getMa().equals(nv.getMa())){
                repo.set(i, nv);
            }
        }
        return "redirect:/nhanVien/index";
